...
typedef struct {    // datos configuración
                ...
                byte aoracc[maxAOR];       // 8= 8 bytes, accion de and/or (0:and, 1:or)
                byte LIBRE7[32];
                byte aortipo[maxAOR]={0,0,0,0,0,0,0,0};     // 8 bytes, 0:Nada, 1:OFF, 2:ON
                ...
                unsigned long timebyday[maxSD]={0,0,0,0,0,0,0,0}; // 4x8=32 bytes
                byte LIBRE8[1415];           // LIBRES para reserva
